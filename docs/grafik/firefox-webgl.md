...
løsningen, risikoerne og en foreslået rækkefølge (aug 2026). Grundlaget er docs/grafik/gpu-historien.md §5.15,
...
- Producenten leverede både `kodi.bin` og en chromium 47 med GLES2-ozone-lag
  ...
  syscall-problem som docs/faeller.md fælde 14). Stadig umålt: hvilken GL-vej den brugte
...
2. **Stock Firefox med `MOZ_X11_EGL=1`** + `LD_LIBRARY_PATH=/opt/hybris` (+ shim).
   ...
   **LØST (25. aug):** 0x3000's rodårsag var, at Firefox' `SymbolLoader`
   ...
   `docs/grafik/gpu-historien.md §5.15c, `docs/faeller.md` fælde 23.
...
5. I mellemtiden: `chromium` + SwiftShader er den eneste browser-vej med WebGL, der
   virker i dag (docs/faeller.md fælde 16) — CPU-fart, men den virker.
...
|---|---|
| GPU-stakken, opskriften og reglerne | `devuan/gpu/README.md`, docs/grafik/gpu-historien.md §5.15 |
| Historien i hverdagssprog | `docs/grafik/hvorfor.md` |
| WebGL-fælden og chromium-nødløsningen | `docs/faeller.md` fælde 16, docs/grafik/gpu-historien.md §5.13-5.14 |
