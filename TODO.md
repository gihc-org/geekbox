...
- [ ] Desktop med GPU/WebGL: hovedvejen er libhybris-stakken — men bemærk: `vendor_root/usr/local/lib` indeholder kun hybris-BROER (libEGL/libGLESv2 + 4 eglplatformer + test_hwcomposer); de rigtige PowerVR-blobs findes KUN i dualOS-imaget V151129 (målt aug 2026: ingen /system-partition i V160309-imaget, blobs'ene overskrevet ved UF-flashen). Kernen har `pvrsrvkm` loadet (målt: `/dev/pvrsrvkm` findes, pvr-kerneltråde kører). Uden KMS/DRI er der dog ingen vej til X — fuld forklaring og trin-stige i DOK §5.13+§5.15. Trin 0-1 (hent blobs + test_hwcomposer) = weekendprojekt; browser-WebGL = uger-måneder. **Alternativ til WebGL-spil:** `chromium` (150.x i armhf-arkivet; egen software-GL SwiftShader, forvent `--no-sandbox` og CPU-fart) — afprøv ved at lægge den i `extra_packages.sh` (installation i qemu-chroot, jf. DOK §5.9). NB (20. aug 2026): direkte installation på boksen brownout-crashede den — men det var strømforsyningen, ikke chromium (DOK §5.14). Med god strøm virker boks-installation; imaget kan dog stadig ikke rumme chromium (rootfs 84 % fuld med firefox)
