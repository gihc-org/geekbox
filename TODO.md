...
- [ ] Desktop med GPU/WebGL: hovedvejen er libhybris-stakken — men bemærk: `vendor_root/usr/local/lib` indeholder kun hybris-BROER (libEGL/libGLESv2); de rigtige PowerVR-blobs ligger i Android-`/system` på eMMC'en. Kernen har `pvrsrvkm` loadet (målt aug 2026: `/dev/pvrsrvkm` findes). Uden KMS/DRI er der dog ingen vej til X — fuld forklaring i DOK §5.13. Research, lav prioritet. **Alternativ til WebGL-spil:** `chromium` (150.x i armhf-arkivet; egen software-GL SwiftShader, forvent `--no-sandbox` og CPU-fart) — afprøv ved at lægge den i `extra_packages.sh` (installation i qemu-chroot, jf. DOK §5.9)
