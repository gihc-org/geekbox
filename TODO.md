...
dd er nødvendig (se docs/boksen/flash.md §10). Scripts i `devuan/`:
...
- [x] Efter-flash-efterbehandling scriptet (aug 2026) — `devuan/emmc_first_boot.sh` (resize2fs + 2 GB swapfil; swapfilen kan ikke ligge i det faste image — se docs/boksen/flash.md §10 "Efter første eMMC-boot"). Samme session: 09's verifikation gjort OOM-sikker (hashing i 8 MiB-bidder) — se docs/boksen/flash.md §10 fælde 7
...
- [ ] Boks 1: opdatér `/root/myinit.sh` til repoets nuværende version ved lejlighed (`ssh -i ~/.ssh/geekbox_key root@<ip> 'cat > /root/myinit.sh' < devuan/myinit.sh`) — dens ældre myinit virker i dag, men kun fordi dens PHY-timing og EDID-race tilfældigvis opfører sig (se docs/boksen/flash.md §10, fælde 3+4)
...
- **Ældre start-prompter** (6. sep 2026 og tidligere) er arkiveret i
  `docs/log/start-prompter-2026-08.md`. De beskriver tilstande der er passeret.
