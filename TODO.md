...
- [ ] **Disken skal udvides efter flash, ellers går boksen i sort** — imaget har et 1,4 GB filsystem; browser-cache fylder det på en aften, og når disken er fuld dør X-sessionen TAVST (tom `.xsession-errors`, ingen logs). `df` hører i de første tre kommandoer man kører. Overvej at lægge `resize2fs` ind i `myinit.sh` (én gang, hvis fs < partition), så `emmc_first_boot.sh` ikke kan glemmes
- [ ] **Ingen syslog-daemon i imaget** — nodm's og andres fejlbeskeder går i ingenting. Læg `sysklogd` (eller `busybox-syslogd`) i `extra_packages.sh`; i aftes fejlsøgte vi i blinde af den grund
...
- [ ] Boks 3 og 4: kør `devuan/emmc_first_boot.sh` (resize2fs 1,4 GB → 15 GB + 2 GB swapfil) — ikke kørt endnu. Og `passwd` for kristian + root (07 sætter midlertidigt `geekbox`)
