...
dd er nødvendig (se docs/DOKUMENTATION.md §10). Scripts i `devuan/`:
...
- [ ] **Kernel-rebuild-regression: WiFi mangler (26. aug 2026)** — vores genbyggede
  ...
  `docs/log/2026-08-26-ddk15-baseline-flashtest.md`.
...
- [x] Efter-flash-efterbehandling scriptet (aug 2026) — `devuan/emmc_first_boot.sh` (resize2fs + 2 GB swapfil; swapfilen kan ikke ligge i det faste image — se docs/DOKUMENTATION.md §10 "Efter første eMMC-boot"). Samme session: 09's verifikation gjort OOM-sikker (hashing i 8 MiB-bidder) — se docs/DOKUMENTATION.md §10 fælde 7
...
- [ ] Nye scripts fra aug 2026 der bør nævnes i docs/DOKUMENTATION.md: `testflash.sh` (byg+flash med pause til loader-tilstand), `find_box.sh` (find boksen på nettet), `rollback.sh` (sæt uboot-logo-flaget tilbage og flash), `fb_overscan.py`, `patch_uboot_logo.py`
...
- [ ] Boks 1: opdatér `/root/myinit.sh` til repoets nuværende version ved lejlighed (`ssh -i ~/.ssh/geekbox_key root@<ip> 'cat > /root/myinit.sh' < devuan/myinit.sh`) — dens ældre myinit virker i dag, men kun fordi dens PHY-timing og EDID-race tilfældigvis opfører sig (se docs/DOKUMENTATION.md §10, fælde 3+4)
...
- **God start i en ny session (cyan-scene SPILBAR, 8. sep 2026 aften):**
  *"Læs `OVERBLIK.md` (opdateret 22:00), derefter
  ...
  `docs/log/2026-08-27-cyan-scene-handover.md` (baggrund + fælder).
  STATUS: Subway Surfers ER SPILBART med proxy vnext12 (`dcc0a68f`, installeret
  på boksen) + alpha-shim (BiDi-preload) + env `CYAN_DEPTHCLEAR=1` +
  depthmask-lap. Dreng/tog/bygninger i korrekt perspektiv; input virker.
  Bring-up efter strøm: `bash devuan/find_box.sh` → ur-sync, insmod
  `pvrsrvkm_leddaz.ko` + `sh gpu_up.sh`, bindapi-lap (sed IP → bash), tjek
  `/dev/sw_sync` 0666, proxy vnext12 (byg fra repo `egl_proxy.c` hvis /tmp
  ryddet: scp → `gcc ... -o /opt/hybris/libEGL.so.1.0.0`).
  START: ren profil (cache2/startupCache/storage/cookies/sessionstore) →
  `GAME_URL=about:blank CYAN_DEPTHCLEAR=1 nohup /root/start_cyan_probe.sh &` →
  `PYTHONUNBUFFERED=1 nohup python3 /root/bidi_cyan.py preload goto
  https://poki.com/en/g/subway-surfers 2400 &` (alpha-shim + attrs-dump) →
  klik play ved 'Press to play'.
  UDESTÅENDE: (1) LAV FPS → byg CYAN_LIGHT (slå predraw/postdraw-readbacks,
  fulde dumps og swapSample fra via env) og sammenlign; (2) lejlighedsvise BLÅ
  glitches (forsvinder hurtigt) — karakterisér; (3) gør alpha-shim + depth-clear
  SELVSTARTENDE (pt. BiDi-preload + env pr. kørsel); (4) rodårsag: hvorfor
  ignorerer 1.5 spillets glClearDepthf(1) (glGetFloatv=0; explicit
  rgl_glClearDepthf(1) før clear virker) — evt. kernel/driver-notes.
  FÆLDER (fortsat gældende): ryd profil før hver load; ingen BiDi-reload under
  load (bot=1); SIGKILL → beskidt profil; hængt lyd = `kill -9 <pulsepid>`;
  /tmp ryddes ved reboot; BiDi = kun 1 session (domcheck kræver genstart efter
  preload-session). Evidens på boks: `/root/cyan_draw_probe_vnext{9_shim2b,10,
  11_play,12_play}_20260908.log`; laptop: `/tmp/fb_play*.raw`+png.
  Commits: `14434ef`, `42ed269` [codex:deepseek-v4-flash]."*
...
- [ ] **Sorte Firefox-chrome (26. aug aften, bagefter konteksttabet):** med
  ...
  `docs/log/2026-08-26-gralloc-lock-spor.md`.
- [ ] **Python-frontend + GLES-daemon (i gang aug 2026)** — Python tegner UI på /dev/fb0 (skelet: `fb_overscan.py`), taler med en C-daemon over unix-socket (skelet: `devuan/gpu/test_triangle.cpp`), daemonen renderer GLES offscreen og blitter til skærmen. Operationskort + første skridt: `devuan/gpu/README.md`. Regler: X stoppet under brug; strøm-cyklus bagefter
  - **God start i en ny session:** *"Læs `devuan/gpu/README.md` og docs/DOKUMENTATION.md §5.15. Vi skal i gang med Python-frontend + GLES-daemon-projektet — arkitekturen og de første skridt står i README'en og i dette TODO-punkt. Boksen findes med `devuan/find_box.sh`."*
...
- [ ] Trin 2 (nyttiggørelse): vendor-kodi-binæren fra 2016 til lokal video (muligvis kørbar nu — brikkerne ligger i `vendor_root`), eller portér en moderne browser (uger-måneder: firefox/chromium kræver EGL-X11/DRI/KMS — løsningsanalyse, rækkefølge og billige eksperimenter: `docs/grafik/firefox-webgl.md`, aug 2026). **Alternativ til WebGL-spil:** `chromium` (150.x i armhf-arkivet; egen software-GL SwiftShader, forvent `--no-sandbox` og CPU-fart) — afprøv ved at lægge den i `extra_packages.sh` (installation i qemu-chroot, jf. docs/DOKUMENTATION.md §5.9). NB (20. aug 2026): direkte installation på boksen brownout-crashede den — men det var strømforsyningen, ikke chromium (docs/DOKUMENTATION.md §5.14). Med god strøm virker boks-installation; imaget kan dog stadig ikke rumme chromium (rootfs 84 % fuld med firefox)
- [ ] **Kernel-sikkerhedsopdatering: 3.10.79 → 3.10.108** — se docs/grafik/driver-portering.md §6.
  ...
  - [x] **Kernel-rebuild er også DDK 1.5-vejen:** 1.5-KM (fra `geekboxzone/mmallow_kernel`,
        ...
        overblik + kommandoer:
        `docs/log/2026-08-26-ddk15-kernel-rebuild-handover.md` + løsningen i
        `docs/log/2026-08-26-ddk15-kernel-rebuild-loesning.md`
